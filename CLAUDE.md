...
- **No inline / dynamic `import()` calls.** All imports must be top-of-file static `import` (or `import type`) statements. Do **not** write `await import("…")`, `const x = await import("…")`, or `import("…").then(…)` anywhere in application code. Dynamic imports hide dependencies from the static graph, defeat tree-shaking and IDE refactors, and are often used as a band-aid for circular dependencies — fix the cycle by restructuring modules instead. The only permitted exceptions are: (a) route-level code-splitting wired through TanStack Router's lazy route APIs, and (b) Vite-native asset imports such as `import.meta.glob`. Both must be explicitly justified in review; no other dynamic imports allowed.
