...
| Comment-like cache strategy | Single `useToggleCommentLike(postId)` hook co-locating both `useLikeComment` + `useUnlikeComment` — mirrors the existing `useTogglePostLike()` convention. New pure helper `patchCommentLike(c, liked, viewer)` returns a new `Comment` with `viewerState.liked` flipped, `counters.likes ± 1`, and the viewer prepended/removed from `likesPreview.preview` (with `count` adjusted, dedup by id, cap top-5). | Mirrors the toggle hook + cache fan-out shape already used for post likes. The comment list patch walks both the top-level and reply caches so a like applied to a reply patches the right cache. |
...
| Hook | Behavior |
|---|---|
| `useToggleCommentLike(postId)` | Single hook co-locating `useLikeComment` + `useUnlikeComment` — mirrors `useTogglePostLike()`. Returns `{ toggle(comment), isPending(commentId) }`. Both mutations: `onMutate` cancels the top-level cache + every reply cache for the post (predicate) + `['comment-likes-preview', commentId]`; snapshots all three; `patchCommentInAllCaches(qc, postId, id, c => patchCommentLike(c, !c.viewerState.liked, viewerSummary))`; `setQueryData(['comment-likes-preview', commentId], prev => prev ? { data: liked ? patchLikesPreviewAddViewer(prev.data, viewerSummary) : patchLikesPreviewRemoveViewer(prev.data, viewerSummary.id) } : prev)`. Both share the same context shape `{ commentSnapshots, previewSnapshot }` so `onError` restores symmetrically + `toast.error("Couldn't update like")` (same string as `useTogglePostLike` for consistency). **No `onSettled` refetch.** |
...
5. `src/features/feed/use-toggle-comment-like.ts` — single hook co-locating like + unlike (mirrors `use-toggle-post-like.ts`).
