...
| `src/components/feed/PostCardReactions.tsx` | Wrap the like-count text in `<LikesPreviewHoverCard kind="post" id={post.id} embedded={post.likesPreview}>{...existing count text...}</LikesPreviewHoverCard>`. No other behavior change — the heart button toggle logic stays. |
...
15. Modify `src/components/feed/PostCardReactions.tsx` — wrap like-count in `<LikesPreviewHoverCard kind="post" />`.
